...
const VehiclesTab = () => {
  ...
  useEffect(() => { if (!editingId) setForm(f => ({...f, workGroupId: state.activeWorkGroupId || f.workGroupId || ''})); }, [state.activeWorkGroupId, editingId]);

  const activeGroupVehicles = state.vehicles.filter(v => !state.activeWorkGroupId || v.workGroupId === state.activeWorkGroupId);
...
const RoutesTab = () => {
  ...
  const activeGroupRoutes = state.routes.filter(r => !state.activeWorkGroupId || r.workGroupId === state.activeWorkGroupId);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.code || form.operatingDays.length === 0 || !form.workGroupId) return;
