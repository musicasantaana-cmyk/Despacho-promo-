...
export const TrackingPage: React.FC = () => {
  ...
  const executeStatusChange = async (status: Assignment['status'], weightTons?: number) => {
    if (!selectedAssignment) return;
    await updateAssignmentStatus(selectedAssignment.id, status, weightTons);
  };
            ...
            <div className="p-4 md:p-6 border-b border-slate-100 bg-slate-50">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center">
                  <button onClick={() => setSelectedAssignmentId(null)} className="mr-3 p-2 -ml-2 rounded-lg text-slate-500 hover:bg-slate-200 lg:hidden">
